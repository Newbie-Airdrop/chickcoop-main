...
def spin_wheel(auth, index):
    ...
    if response.status_code == 200:
        ...
        if wheel_state.get('availableReward'):
            ...
            print(f"account {index} You got {reward['text']} | {reward['type']} | {reward['amount']}")
            
            # Claim the reward
            claim_response = requests.post('https://api.chickcoop.io/wheel/claim', headers=headers)
            if claim_response.status_code == 200:
                print(f"account {index} Reward claimed successfully")
            else:
                print(f"account {index} Failed to claim reward")
        else:
            ...
            if next_spin_time:
                ...
                print(f"account {index} Next spin in {hours} hours {minutes} minutes {seconds} seconds")
        return True
    return False
    ...
    while True:
        try:
            ...
            if response.status_code == 401:
                return Fore.RED + f"Authorization failed for account {index + 1}"
            ...
            result = (
                f"account {Style.BRIGHT}{color}{index + 1}{Style.RESET_ALL} | "
                f"Số gà {Style.BRIGHT}{color}{chicken_quantity} ({chicken_percentage:.0f}%){Style.RESET_ALL} | "
                f"Level {Style.BRIGHT}{color}{level}{Style.RESET_ALL} | "
                f"Trứng {Style.BRIGHT}{color}{int(eggs['quantity'])}{Style.RESET_ALL} | "
                ...
                f"User Name: {Style.BRIGHT}{color}{profile['username']}{Style.RESET_ALL}"
            )
            ...
            print(Fore.RED + f"Error fetching data for account {index + 1}: {e}")
